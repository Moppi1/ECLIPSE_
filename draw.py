...
def image(path:str,pos : vec_2d.vec , s=1 , r=0):
    """prints an image at a vec pos"""
    im = pygame.image.load(r""+path)
    h = im.get_height()* s /2  #for centering
    w = im.get_width()* s /2   # "
    im = pygame.transform.rotozoom(im.convert(),r,s) # zooming
    screen.blit(im,((pos.x + width // 2)-w,((pos.y - height // 2)*-1)-h))
